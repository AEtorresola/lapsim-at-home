...
from tire_class import EnhancedTire
...
def test_constant_speed():
    ...
    tire = EnhancedTire(mf_tire, position="front_left", radius=0.33, inertia=1.5, smoothing_factor=0.3)
    ...
    Fx_array = np.zeros(steps)
    Fy_array = np.zeros(steps)
    max_Fx_array = np.zeros(steps)
    ...
    for i in range(steps):
        ...
        Fx_array[i] = forces['Fx']
        Fy_array[i] = forces['Fy']
        max_Fx_array[i] = forces['max_Fx']
    ...
    # Plot forces
    plt.subplot(3, 1, 3)
    plt.plot(time_array, Fx_array, label='Fx (Applied)')
    plt.plot(time_array, max_Fx_array, label='Max Available Fx', linestyle='--')
    plt.plot(time_array, Fy_array, label='Fy', color='g')
    plt.xlabel('Time (s)')
    plt.ylabel('Force (N)')
...
def test_simple_track():
    ...
    for i in range(steps):
        ...
        for j in range(len(simple_track) - 1):
            ...
            if pos1 <= current_position <= pos2:
                ...
                t = (current_position - pos1) / (pos2 - pos1) if pos2 != pos1 else 0
                target_speed = speed1 + t * (speed2 - speed1)
                Fy_desired = lat1 + t * (lat2 - lat1)
                break
    ...
    plt.subplot(3, 1, 2)
    plt.plot(position_array, Fx_array, label='Fx (Applied)')
    plt.plot(position_array, max_Fx_array, 'b--', label='Max Available Fx')
    plt.plot(position_array, Fy_array, 'g', label='Fy (Applied)')
